...
class BaseWebsite(QObject):
    ...
    def add_chapter_download_record(self, chapters):
        for name in chapters:
            ...
            chapter.total_images = self.get_chapter_pages_number(name)
            chapter.downloaded_images = 0
            # 写入数据库
            chapter.id = self.chapter_task_mapper.insert(chapter)
            self.add_image_download_record(chapter.id, chapter.save_path+"/"+name,chapter.total_images)
            self.addChapterDownloadRecordFinished.emit(chapter)
        
    def add_image_download_record(self, chapter_id,output_path,total_images):
        image = ImageTask()
